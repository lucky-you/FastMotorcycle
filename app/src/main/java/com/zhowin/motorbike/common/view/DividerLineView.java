...
public class DividerLineView extends View {
    ...
    public DividerLineView(Context context, AttributeSet attrs) {
        ...
        int color;
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.DividerLineView, 0, 0);
        try {
            dashGap = a.getDimensionPixelSize(R.styleable.DividerLineView_dashGap, 5);
            dashLength = a.getDimensionPixelSize(R.styleable.DividerLineView_dashLength, 5);
            dashThickness = a.getDimensionPixelSize(R.styleable.DividerLineView_dashThickness, 3);
            color = a.getColor(R.styleable.DividerLineView_divider_line_color, 0xff000000);
            orientation = a.getInt(R.styleable.DividerLineView_divider_orientation, ORIENTATION_HORIZONTAL);
        } finally {
            a.recycle();
        }
        mPaint = new Paint();
